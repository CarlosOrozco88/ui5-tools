...
import fsutil from 'ui5-nwabap-deployer-core/lib/FileStoreUtil';
import AdtClient from 'ui5-nwabap-deployer-core/lib/AdtClient';
import { XmlDocument } from 'xmldoc';

const CTS_BASE_URL = '/sap/bc/adt/cts/transports';
const CTS_BASE_URL_TRR = '/sap/bc/adt/cts/transportrequests?targets=true&user=corozco';
 ...
 * @param {object}  oOptions.auth               auth
...
TransportManager.prototype.createTransport = function (sPackageName, sRequestText, fnCallback) {
  ...
  this._client.determineCSRFToken(
    function () {
      ...
      this._client.sendRequest(
        ...
        function (oError, oResponse) {
          if (oError) {
            ...
            fnCallback(
              new Error(
                `Operation Create Transport: Expected status code ${fsutil.HTTPSTAT.ok}, actual status code ${oResponse.statusCode}`
              )
            );
            return;
          } else {
            ...
            this._oLogger.log(
              'Creation of transport request required. Number of created transport request: ' + sTransportNo
            );
...
TransportManager.prototype.determineExistingTransport = function (fnCallback) {
  ...
  const sUrl2 = this._client.buildUrl(CTS_BASE_URL);
